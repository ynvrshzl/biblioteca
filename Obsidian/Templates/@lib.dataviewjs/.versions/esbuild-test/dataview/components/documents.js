...
const docfiles = pages.filter(p => p.file.folder.includes('doc'));
...
if (docfiles.length === 0) {
    ...
    docfiles.map((p, i) => {
        ...
        const name = dv.el('span', p.file.name + ".md");
        name.style.cssText = 'color: var(--text-normal); ';


        // const dsc = dv.el('span', p.file.mtime.toFormat('EEEE, h:mm a'));
        const dsc = dv.el('span', Math.round(p.file.size / 1024) + " MB");
